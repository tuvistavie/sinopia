...
function Auth(config) {
  ...
  self.plugins.push({
    ...
    allow: function (user, action, package, cb) {
      cb( Error[403]('user ' + user.name + ' is not allowed to ' + action) )
    },
...
Auth.prototype.allow = function (user, action, package_name, callback) {
  var plugins = this.plugins.slice(0)
  var package = null
  if (!callback && typeof package_name === 'function') {
    callback = package_name
    package_name = null
  } else {
    package = Object.assign({ name: package_name },
  ...
  ;(function next() {
    ...
    if (typeof p['allow_' + action] === 'function') {
      m = p['allow_' + action].bind(p, user, package, cb)
    } else if (typeof p.allow === 'function') {
      m = p.allow.bind(p, user, action, package, cb)
    }
...
Auth.prototype.allow_access = function(package_name, user, callback) {
  return this.allow(user, 'access', package_name, callback)
}

Auth.prototype.allow_publish = function(package_name, user, callback) {
  return this.allow(user, 'publish', package_name, callback)
}
